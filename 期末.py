...
if choice == choices[2] :                              ##'小台指期貨2024.12到期: 2023.12 至 2024.4.11':
    df_original = load_data('future_KBar_CDF')
...
def Change_Cycle(Date,cycle_duration,KBar_dic,product_name):
    ...
    if len(KBar_dic['time']) == 0:
        st.warning("原始 KBar 資料為空，請確認資料來源或參數設定。")
        return {
            'time': np.array([]),
            'product': np.array([]),
            'open': np.array([]),
...
if len(OrderRecord.Profit)>0:
	交易總盈虧 = trades['損益'].sum()
	平均每次盈虧 = trades['損益'].mean()
	平均投資報酬率 = trades['報酬率'].mean()
	平均獲利_只看獲利的 = trades[trades['損益'] > 0]['損益'].mean()
	平均虧損_只看虧損的 = trades[trades['損益'] < 0]['損益'].mean()
	勝率 = len(trades[trades['損益'] > 0]) / len(trades)
	最大連續虧損 = ...（自行撰寫邏輯）
	最大盈虧回落_MDD = ...（MDD計算邏輯）
	報酬風險比 = 平均每次盈虧 / trades['損益'].std()
    data = {
